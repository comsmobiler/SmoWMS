            ...
            try
            {
                MessageBox.Show("你确定要该行项吗?", "系统提醒", MessageBoxButtons.OKCancel, (object sender1, MessageBoxHandlerArgs args) =>
                {
                    try
                    {
                        ...
                        {
                            if (this.Form.ToString() == "SMOWMS.UI.AssetsManager.frmAssSalesOrderCreate")
                            {
                                ((frmAssSalesOrderCreate)Form).RemoveTemplate(LblTId.BindDataValue.ToString());
                            }
                            else
                            {
                                ((frmAssSalesOrderEdit)Form).RemoveTemplate(LblTId.BindDataValue.ToString());
                            }
                        }
                    }
                    catch (Exception ex)
